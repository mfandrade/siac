...
/**
 * Teste do método de geração do número do documento.
 */
...
/**
 * Teste do método de geração do nosso documento.
 */
...
/**
 * Teste do método de cálculo de diferença de datas.
 */
	function testDateDiff() {
		...
		$bissexto	= $this->Boleto->dateDiff('28/02/2008', '01/03/2008');
		$this->assertEqual($bissexto, 2);


		$semana		= $this->Boleto->dateDiff(date('d/m/Y'), date('d/m/Y', strtotime('+1 week')), false);
		$this->assertEqual($semana, -7);

		$semana		= $this->Boleto->dateDiff(date('d/m/Y', strtotime('+1 week')), date('d/m/Y'), false);
		$this->assertEqual($semana, 7);

		$ano		= $this->Boleto->dateDiff('31/12/2008', '31/12/2009', false);
		$this->assertEqual($ano, -365);

		$ano		= $this->Boleto->dateDiff('31/12/2009', '31/12/2008', false);
		$this->assertEqual($ano, 365);

		$bissexto	= $this->Boleto->dateDiff('28/02/2008', '01/03/2008', false);
		$this->assertEqual($bissexto, -2);

		$bissexto	= $this->Boleto->dateDiff('01/03/2008', '28/02/2008', false);
		$this->assertEqual($bissexto, 2);

	}

/**
 * Teste do método de cálculo de data no formato Juliano.
 */
...
/**
 * Teste do método de conversão de formatos de data.
 */
...
/**
 * Teste do método de geração do código de barras.
 */
