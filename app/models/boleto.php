 ...
 *
 * @param integer quadra	número da quadra com dois dígitos
 * @param integer lote		número do lote com dois dígitos
 * @param integer mes		mês em questão com dois dígitos
 * @param integer ano		ano em questão com dois dígitos
 * @param integer tipo		tipo de lancamento (0- taxacondominial, 1- taxaextra, 2-multa, 3- acordo)
 *
 * @return string 	um número de documento com 13 dígitos e zero padded ou false caso os parâmetros sejam loucos
 */
	function gerarNumeroDocumento($quadra, $lote, $mes, $ano, $tipo= TIPO_LANCAMENTO_TAXACONDOMINIAL) {
 ...
 *
 * @param string numero_documento	o número de documento (possivelmente gerado a partir do método dado acima)
 * @param string agencia			o número da agência (os 6 dígitos do código do cedente - sem o "8" dado - ver manual da cnr)
 * @param string vencimento			a data de vencimento, no formato dd/mm/aa (ou dd/mm/aaaa)
 *
 * @return string 	o nosso número (assim chamado o número do documento acrescido de três dígitos) ou false caso
 */
...
/**
 * Calcula a diferença, em dias, entre duas datas no formato humano.
 *
 * @param string  $data1	uma data
 * @param string  $data2	outra data
 * @param boolean $absolute	se false, o resultado é negativo se $data1 for menor que $data2
 *
 * @return integer	a quantidade dias entre a data1 e a data2, ou false caso não sejam datas que existam
 */
	function dateDiff($data1, $data2, $absolute= true) {
		...
		$time1	= mktime(0, 0, 1, $mes1, $dia1, $ano1);
		$time2	= mktime(0, 0, 1, $mes2, $dia2, $ano2);
