...
class BoletoPdfComponent extends Object {
	...
	function __trataDados(&$dados) {
		...
		if( !array_key_exists('parcela', $dados) || empty($dados['parcela']) ) {
			$dados['parcela']	= '001/001';
		}
		if( !array_key_exists('especie', $dados) || empty($dados['especie']) ) {
			$dados['especie']	= ESPECIE_MOEDA_REAL . ' - REAL';
		}
		if( !array_key_exists('carteira', $dados) || empty($dados['carteira']) ) {
			$dados['carteira']	= 'CNR';
		}
		if( !array_key_exists('quantidade', $dados) || empty($dados['quantidade']) ) {
			$dados['quantidade']	= '';
		}
		if( !array_key_exists('valor_unitario', $dados) || empty($dados['valor_unitario']) ) {
			$dados['valor_unitario']= '';
		}

		if( !array_key_exists('instrucoes', $dados) || empty($dados['instrucoes']) ) {
			$dados['instrucoes']	= '';
		} else {
			$dados['instrucoes']	= str_replace('<br />', "\n", $dados['instrucoes']);
		}
		$dados['endereco_completo_sacado']	= sprintf("%s\n%s\n%s - %s - %s, %s", $dados['sacado'] . ' - ' . $dados['sacado_complemento'], $dados['sacado_endereco'], $dados['sacado_cep'], $dados['sacado_bairro'], $dados['sacado_cidade'], $dados['sacado_uf']);
	}
	...
	function __primeiraParte($dados) {
		...
		$this->tcpdf->Cell(100, DEFAULT_CELL_H, sprintf('%s     (%s)', $dados['cedente'], $dados['cpf_cnpj_cedente']), 'BR', 0, 'L');

		$this->tcpdf->SetFont('helvetica', '', 5);
		$this->tcpdf->Text(110+.5, 3*DEFAULT_CELL_H-2-.4, 'Agência/Código Cedente');
		$this->tcpdf->SetFont('helvetica', 'B', 8);
		$this->tcpdf->Cell(25, DEFAULT_CELL_H, $dados['agencia_cod_cedente'], 'BR', 0, 'C');
		...
		$this->tcpdf->SetFont('helvetica', 'B', 8);
		$this->tcpdf->Cell(100, DEFAULT_CELL_H, $dados['sacado'] . ' - ' . $dados['sacado_complemento'], 'BR', 0, 'L');

		$this->tcpdf->SetFont('helvetica', '', 5);
		$this->tcpdf->Text(110+.5, 4*DEFAULT_CELL_H-2-.4, 'Vencimento');
		$this->tcpdf->SetFont('helvetica', 'B', 8);
		$this->tcpdf->Cell(25, DEFAULT_CELL_H, $dados['data_vencimento'], 'BR', 0, 'C');
	...
	function __segundaParte($dados) {
		...
		// linha 1 - logo, codigo banco e titulo da parte
		$this->tcpdf->Cell(30, DEFAULT_CELL_H, '', 'RB', 0, 'L');
		$this->tcpdf->Image($this->caminhoImagens . '/' . $this->bancoLogo, 10, 7*DEFAULT_CELL_H, 29);
		$this->tcpdf->SetFont('helvetica', 'B', 16);		// fonte codigo banco
		$this->tcpdf->Cell(20, DEFAULT_CELL_H, $this->bancoCodigo . '-' . ESPECIE_MOEDA_REAL, 'RB', 0, 'C');
		$this->tcpdf->SetFont('helvetica', 'B', 10);		// fonte subtitulo
		$this->tcpdf->Cell(140, DEFAULT_CELL_H, 'Recibo do Sacado', 'B', 0, 'R');
		$this->tcpdf->Ln();
		...
		$this->tcpdf->SetFont('helvetica', 'B', 8);
		$this->tcpdf->Cell(15, DEFAULT_CELL_H, '', 'BR', 0, 'C');
	...
	function __terceiraParte($dados) {
		...
		$this->tcpdf->Image($this->caminhoImagens . '/' . $this->bancoLogo, 10, 24*DEFAULT_CELL_H-3, 29);
		$this->tcpdf->SetFont('helvetica', 'B', 16);		// fonte codigo banco
		...
		$this->tcpdf->Ln();
		//$this->tcpdf->Image($this->caminhoImagens . '/' . '/codigobarras.jpg', 10, 37*DEFAULT_CELL_H-3, 103, 13);
	...
	function download($filename) {
		...
		return $this->tcpdf->Output($filename, 'D');
	}

	function saveFile($filename) {
		return $this->tcpdf->Output($filename, 'F');
	}
	...
	function __imprimeCodigoBarras($x, $y, $codigo) {
		...
		$pos= $x;
		//Guarda inicial
		$this->tcpdf->Image( $this->caminhoImagens . '/' . 'p.png', $pos, $y, $fino, $altura); $pos+= $fino;
		$this->tcpdf->Image( $this->caminhoImagens . '/' . 'b.png', $pos, $y, $fino, $altura); $pos+= $fino;
		$this->tcpdf->Image( $this->caminhoImagens . '/' . 'p.png', $pos, $y, $fino, $altura); $pos+= $fino;
		$this->tcpdf->Image( $this->caminhoImagens . '/' . 'b.png', $pos, $y, $fino, $altura); $pos+= $fino;
		...
		while( strlen($texto) > 0 ) {
			...
			for( $i= 1; $i < 11; $i+= 2 ) {
				...
				$this->tcpdf->Image( $this->caminhoImagens . '/' . 'p.png', $pos, $y, $f1, $altura); $pos+= $f1;
				...
				$this->tcpdf->Image( $this->caminhoImagens . '/' . 'b.png', $pos, $y, $f2, $altura); $pos+= $f2;
			}
		}
		// Draw guarda final
		$this->tcpdf->Image( $this->caminhoImagens . '/' . 'p.png', $pos, $y, $largo, $altura); $pos+= $largo;
		$this->tcpdf->Image( $this->caminhoImagens . '/' . 'b.png', $pos, $y, $fino, $altura); $pos+= $fino;
		$this->tcpdf->Image( $this->caminhoImagens . '/' . 'p.png', $pos, $y, $fino, $altura); $pos+= $fino;
	}
}
